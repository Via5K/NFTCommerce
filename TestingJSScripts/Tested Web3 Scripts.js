	...
	var result = await nftcontract.methods.viewNFTPropByIndex2(pid).call(
		...
		function (err, res) {
			if (err) {
				...
				if (res[3] == true) document.getElementById('__expireable').innerText = 'True';
				else {
...
export async function createNFT() {
	...
	var result = await callerContract.methods.createNFT(cost, pid, UqUrl, name, description, url, trxnHash, sellerAddress, validTill).send(
		{
			from: account,
		},
		function (err, result) {
